...
WbMem::WbMem(Sim& sim, Sim::Clock* clk, WbSlavePorts ports, MemRead rd,
             MemWrite wr, void* opaque, uint32_t base, size_t size_bytes)
    : sim_(sim), p_(ports), cb_rd_(rd), cb_wr_(wr), cb_opaque_(opaque),
      size_(size_bytes), base_(base) {
  attach(clk);
}
...
void WbMem::clear(uint8_t pattern) {
  if (!data_) return;      // somebody else's memory is not ours to clear
...
uint32_t WbHost::read32(uint32_t adr, uint8_t sel) {
  adr_ = adr;
  wdata_ = 0;
  sel_ = sel;
  write_ = false;
